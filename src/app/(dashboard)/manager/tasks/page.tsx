...
export default async function ManagerTasksPage({
  ...
  // Get project IDs managed by this manager
  ...
  const [tasks, total, users] = await Promise.all([
    ...
    // Fetch members assigned to the manager's projects
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardHeader
        title="Tasks"
        subtitle="Manage tasks under your projects."
      />
      ...
      <form method="GET" className="glass-panel p-4 rounded-xl flex flex-wrap gap-4 items-end shadow-sm">
        ...
        <div className="flex gap-2">
          ...
          {(search || status || priority || assigneeId || deadlineStatus || sortBy !== "dueDate" || sortOrder !== "asc") && (
            <Link href="/manager/tasks" className="btn btn-outline py-1.5 px-4 text-sm h-[38px] flex items-center justify-center">
              Reset
            </Link>
          )}
        </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            accessor: (t) => (
              <Link
                href={`/member/projects/${t.projectId}`}
